<template>
    <div class="item" name="home-item" :finished="finished">
        ...
        <div class="controler" ref="controls">
            <button class="edit">
...
import type { Reminder } from '@/store/reminder-list'
import Checkbox from './checkbox.vue'
import useRemindersList from '@/store/reminder-list'
import { onMounted, ref } from 'vue'
const props = defineProps<Reminder & { index: number }>()
...
const controls = ref<HTMLElement>(null!)

class ItemSideSlip {
    private element: HTMLElement = undefined as any
    private controls: HTMLElement = undefined as any
    private mouseDefaultLocation = 0
    private elementDefaultMoveLocation = 0
    private touched: boolean = false
    private maxMove = -150
    private defaultTransition: string = ''
    constructor(element: HTMLElement, controls: HTMLElement) {
        this.element = element
        this.controls = controls
    }
    getNowLocation(element: HTMLElement) {
        return Number(getComputedStyle(element).left.match(/(.*?)px/)?.[1]) as number
    }
    init() {
        this.defaultTransition = getComputedStyle(this.element).transition as string
        this.element.addEventListener("touchstart", e => {
            this.mouseDefaultLocation = e.touches[0].clientX
            this.elementDefaultMoveLocation = this.getNowLocation(this.element)
            this.touched = true
        })
        window.addEventListener("touchmove", e => {
            ...
            const rst = clientX - this.mouseDefaultLocation + this.elementDefaultMoveLocation
            this.element.style.left = `${rst}px`
            function _rangeMap(min: number, max: number, newmin: number, newmax: number, value: number): number {
                const rst = (value - min) / (max - min) * (newmax - newmin) + newmin
                if (rst <= newmin) return newmin
                else if (rst >= newmax) return newmax
                else return rst
            }
            this.controls.style.opacity = (1 - _rangeMap(-150, 0, 0, 1, rst)).toString()
        })
        window.addEventListener("touchend", () => {
            if (!this.touched) return
            if (this.touched) this.touched = false
            this.element.style.transition = this.defaultTransition
            const nowLocation = this.getNowLocation(this.element)
            if (nowLocation > this.maxMove + 50) {
...
onMounted(() => {
    const itemSlideSlip = new ItemSideSlip(element.value, controls.value)
    itemSlideSlip.init()
})
...
.item {
    ...
    .main-item {
        ...
        box-shadow: 5px 6px 10px #88888836;
        border-radius: 20px;
        align-items: center;
        justify-content: left;
        transition: transform 0.3s, left 0.3s cubic-bezier(0, 0, 0, 0.99);
        position: absolute;
        top: 0;
        left: 0;
        width: 100%;
    ...
    .controler {
        ...
        button {
            ...
            &.edit {
                margin: 0px 10px;
                box-shadow: 2px 3px 11px #ffc403b2;
            ...
            &.delete {
                ...
                color: red;
                box-shadow: 2px 3px 11px #FF7B7B;
            }
        }
    }
}
